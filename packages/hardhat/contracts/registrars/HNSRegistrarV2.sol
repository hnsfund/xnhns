...
import { DepositRegistrar } from "./DepositRegistrar.sol";
 ...
 */
contract HNSRegistrarV2 is IHNSRegistrar, BaseRegistrar, DepositRegistrar {
  constructor(
    IENS _ens,
  ...
  function migrate(bytes32 node, address to) external onlyNFTLDOwner {
    super.preMigrationHook(node, address(this), msg.sender);
    _migrate(node);
    super.postMigrationHook(node, address(this), msg.sender);
  }
